(function (exports) {
  ...
  const FixedTest = () => {
    ...
    const thisEnd = $('.footer').outerHeight();
    const windowH = $(window).height();
    const docuH = $(document).height();
    console.log('windowH', windowH);
    console.log('docuH', docuH);
    $(window).scroll(function () {
      const scrollTop = $(window).scrollTop();
      console.log('docuH', docuH, 'windowH', windowH)
      console.log(scrollTop, docuH - windowH - thisEnd)
      if(scrollTop > docuH - windowH - thisEnd) {
        fixedElem.css('bottom', `${thisEnd + 10}px`);
      } else {
